...
function addBackButtons(){
  document.querySelectorAll('.view').forEach(view=>{
    ...
    const b=document.createElement('button');b.type='button';b.className='pageBack secondary';b.textContent='← Back';b.setAttribute('aria-label','Back to the manuscript');b.addEventListener('click',()=>closeElement(view));card.insertBefore(b,card.firstChild);
  });
...
function outsideClick(e){
  ...
  document.querySelectorAll('.drawer.open,.chat.open').forEach(el=>{if(!el.contains(e.target))closeElement(el)});
}
function keydown(e){if(e.key!=='Escape')return;for(const selector of CLOSE_SELECTORS){const el=document.querySelector(selector+'.open')||(selector==='#textRecordView'?document.querySelector('#textRecordView[style*="block"]'):null);if(el){closeElement(el);break}}}
function styles(){if(document.getElementById('oracleNavigationStyles'))return;const s=document.createElement('style');s.id='oracleNavigationStyles';s.textContent=`.pageBack{display:inline-flex!important;align-items:center;gap:6px;margin-bottom:16px}.pageBack:focus-visible{outline:3px solid #8d342c;outline-offset:3px}.view>.workspace{position:relative}`;document.head.appendChild(s)}
