...
(() => {
  const message =
    'The Oracle is online, but the free AI provider is unavailable. The interface is working. Add a free Gemini API key to the Supabase Edge Function (GEMINI_API_KEY), or restore a working Pollinations provider, then try again.';
  ...
  const cleanOldError = () => {
    document.querySelectorAll('#chatLog .bubble.assistant').forEach((node) => {
      ...
      if (
        window.oracleCitations &&
        !node.dataset.citationsRendered &&
        /\[\[cite:[0-9a-fA-F-]{36}\]\]/.test(node.textContent || '')
      ) {
