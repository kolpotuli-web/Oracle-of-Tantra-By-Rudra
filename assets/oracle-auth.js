...
async function profileSync(user) {
  const name =
    localStorage.getItem('oracleName') ||
...
async function restoreNotes(user) {
  ...
  localStorage.setItem(
    'oracleNotes',
    JSON.stringify(
      data.map((n) => ({ title: n.title, body: n.body, time: Date.parse(n.created_at) })),
    ),
  );
...
async function updateAuthUI() {
  ...
  $('authSubmit').style.display = user && !recoveryMode ? 'none' : 'inline-block';
  $('authSignup').style.display = user || recoveryMode ? 'none' : 'inline-block';
  $('authForgot').style.display = user || recoveryMode ? 'none' : 'inline-block';
  $('authGoogle').style.display = user || recoveryMode ? 'none' : 'inline-block';
  $('authPrefs').style.display = user && !recoveryMode ? 'block' : 'none';
  $('authTitle').textContent = recoveryMode
    ? 'Set a new password'
    : user
      ? 'Researcher account'
      : 'Enter the library';
  $('authIntro').textContent = recoveryMode
    ? 'Choose a new password for your researcher account.'
    : user
      ? 'Manage your researcher workspace and account preferences.'
      : 'Create a private researcher account to sync notes, bookmarks and conversations across devices.';
  $('authUser').textContent =
    user && !recoveryMode
      ? `Signed in as ${user.email}. Your private workspace can sync across devices.`
      : '';
  if (user && !recoveryMode) {
    const { data: p } = await supabase
      ...
      .eq('id', user.id)
      .maybeSingle();
    $('authDisplayName').value = p?.display_name || user.user_metadata?.full_name || '';
    $('authLanguage').value = p?.preferred_language || localStorage.getItem('oracleLang') || 'en';
  }
...
async function signIn() {
  ...
  await profileSync(data.user);
  await restoreNotes(data.user);
  message('Signed in.');
  await updateAuthUI();
}
async function signUp() {
  ...
  if (!email || password.length < 8)
    return message('Use a valid email and a password of at least 8 characters.', true);
  const { data, error } = await supabase.auth.signUp({ email, password });
  if (error) return message(error.message, true);
  if (data.user) {
    await profileSync(data.user);
    await syncLocalNotes(data.user);
    message(
      data.session
...
async function forgot() {
  const email = $('authEmail').value.trim();
  if (!email) return message('Enter your email first, then choose Forgot password.', true);
...
async function changePassword() {
  ...
  if (password.length < 8) return message('Use a password of at least 8 characters.', true);
...
async function savePrefs() {
  ...
  const r = document.getElementById('researchLanguage');
  if (r) r.value = lang;
  const status = document.getElementById('status');
  if (status) status.textContent = name + ' · synced workspace';
  message('Researcher settings saved.');
}
$('authX').onclick = closeAuth;
$('authSubmit').onclick = () => (recoveryMode ? changePassword() : signIn());
$('authSignup').onclick = signUp;
$('authForgot').onclick = forgot;
$('authGoogle').onclick = google;
$('authSignout').onclick = signOut;
$('authSavePrefs').onclick = savePrefs;
...
supabase.auth.onAuthStateChange(async (event, session) => {
  ...
  if (session?.user && !recoveryMode) {
    await profileSync(session.user);
    await syncLocalNotes(session.user);
    await restoreNotes(session.user);
  }
  await updateAuthUI();
});
updateAuthUI();
