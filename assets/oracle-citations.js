...
function ensureStyles(){
  ...
  const s=document.createElement('style'); s.id='oracleCitationStyles';
  s.textContent=`
...
function ensureDrawer(){
  ...
  const d=document.createElement('aside'); d.id='citationDrawer'; d.className='citationDrawer';
  ...
  document.body.appendChild(d); document.getElementById('citationClose').onclick=()=>d.classList.remove('open');
}
...
async function openCitation(id){
  ensureDrawer(); const body=document.getElementById('citationBody'), d=document.getElementById('citationDrawer');
  d.classList.add('open'); body.innerHTML='<p>Loading source record…</p>';
  try{
    const c=await getCitation(id);
    if(!c){body.innerHTML='<p>That citation is unavailable in this researcher workspace.</p>';return;}
    const pages=(c.page_start||c.page_end)?` · pages ${c.page_start??'—'}–${c.page_end??'—'}`:'';
  ...
  }catch(e){body.innerHTML=`<p>Unable to load this source record.</p><p class="citationNotice">${esc(e.message)}</p>`}
}
...
function renderCitations(container){
  ...
  const html=esc(source).replace(pattern,(_,id)=>`<button type="button" class="citationRef" data-citation="${id}">source</button>`);
  container.innerHTML=html.replace(/\n/g,'<br>');
  const ids=[...container.querySelectorAll('[data-citation]')].map(x=>x.dataset.citation);
  if(ids.length){
    const shelf=document.createElement('div'); shelf.className='citationShelf'; shelf.innerHTML='<div class="citationShelfLabel">Sources referenced in this answer</div>'+[...new Set(ids)].map(id=>`<button type="button" class="citationRef" data-citation="${id}">${esc(id.slice(0,8))}</button>`).join(''); container.appendChild(shelf);
  }
}

document.addEventListener('click',e=>{const b=e.target.closest('[data-citation]');if(b)openCitation(b.dataset.citation)});
