...
async function loadSources(){
  ...
  const {data,error}=await supabase.from('source_submissions').select('title,author,source_type,rights_status,review_status,url,created_at').order('created_at',{ascending:false}).limit(50);
  if(error){list.innerHTML='<p class="sourceHint">Unable to load submissions.</p>';return}
  list.innerHTML=data?.length?data.map(r=>`<div class="sourceRow"><h3>${escapeHtml(r.title)}</h3><p>${escapeHtml(r.author||'Unknown author')} · ${escapeHtml(r.source_type)} · ${escapeHtml(r.rights_status)}</p><div class="sourceStatus">${escapeHtml(r.review_status)}</div>${r.url?`<p><a href="${escapeAttr(r.url)}" target="_blank" rel="noopener">Open lawful source</a></p>`:''}</div>`).join(''):'<p class="sourceHint">No submissions yet.</p>';
}
