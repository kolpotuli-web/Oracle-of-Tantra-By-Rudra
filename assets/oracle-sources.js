...
function addNav(){
  const rail=document.querySelector('.rail'); if(!rail || rail.querySelector('[data-source-nav]')) return;
  const b=document.createElement('button'); b.type='button'; b.textContent='Sources'; b.dataset.sourceNav='1'; rail.appendChild(b); b.addEventListener('click',openView);
}
function openView(){document.getElementById('oracleSources')?.classList.add('open');loadSources()}
function closeView(){document.getElementById('oracleSources')?.classList.remove('open')}
function build(){
  injectStyles(); addNav();
  const v=document.createElement('section'); v.id='oracleSources'; v.className='sourceView';
...
async function uploadPrivateFile(user, file, messageEl){
  ...
  if(file.size>maxBytes){messageEl.textContent='Private files are limited to 25 MB in this free build.';return '__ERROR__';}
  const safeName=file.name.replace(/[^a-zA-Z0-9._-]/g,'_');
  const path=`${user.id}/${crypto.randomUUID()}-${safeName}`;
  const {error}=await supabase.storage.from('oracle-private-sources').upload(path,file,{cacheControl:'3600',upsert:false});
  if(error){messageEl.textContent='File upload failed: '+error.message;return '__ERROR__';}
  return path;
}

async function submit(e){
  e.preventDefault(); const {data:{user}}=await supabase.auth.getUser(); const m=document.getElementById('sourceMessage');
  if(!user){m.textContent='Sign in from Profile before submitting a source.';return}
  const file=document.getElementById('sourceFile').files?.[0]||null;
  const storagePath=await uploadPrivateFile(user,file,m); if(storagePath==='__ERROR__')return;
  const payload={user_id:user.id,title:document.getElementById('sourceTitle').value.trim(),author:document.getElementById('sourceAuthor').value.trim()||null,source_type:document.getElementById('sourceType').value,url:document.getElementById('sourceUrl').value.trim()||null,rights_status:document.getElementById('sourceRights').value,description:document.getElementById('sourceNotes').value.trim()||null,storage_path:storagePath};
  if(!payload.title){m.textContent='A title is required.';return}
  const {error}=await supabase.from('source_submissions').insert(payload); if(error){if(storagePath)await supabase.storage.from('oracle-private-sources').remove([storagePath]);m.textContent=error.message;return}
  m.textContent='Submitted. Review status: pending.'; document.getElementById('sourceForm').reset(); loadSources();
}

async function loadSources(){
  const list=document.getElementById('sourceList'); if(!list)return; const {data:{user}}=await supabase.auth.getUser();
  if(!user){list.innerHTML='<p class="sourceHint">Sign in from Profile before submitting or viewing your private source submissions.</p>';return}
  const {data,error}=await supabase.from('source_submissions').select('title,author,source_type,rights_status,review_status,url,storage_path,created_at').order('created_at',{ascending:false}).limit(50);
  if(error){list.innerHTML='<p class="sourceHint">Unable to load submissions.</p>';return}
  list.innerHTML=data?.length?data.map(r=>`<div class="sourceRow"><h3>${escapeHtml(r.title)}</h3><p>${escapeHtml(r.author||'Unknown author')} · ${escapeHtml(r.source_type)} · ${escapeHtml(r.rights_status)}</p><div class="sourceStatus">${escapeHtml(r.review_status)}</div>${r.url?`<p><a href="${escapeAttr(r.url)}" target="_blank" rel="noopener">Open lawful source</a></p>`:''}${r.storage_path?'<p>Private file attached</p>':''}</div>`).join(''):'<p class="sourceHint">No submissions yet.</p>';
}
