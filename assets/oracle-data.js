...
async function ensureConversation(firstMessage = '') {
  ...
  if (!user) return null;
  const title =
    (firstMessage || 'Oracle research conversation').trim().slice(0, 90) ||
    'Oracle research conversation';
  const payload = {
    user_id: user.id,
    title,
    language: localStorage.getItem('oracleLang') || 'en',
    project_id: window.oracleActiveProjectId || null,
  };
...
async function syncNotes() {
  ...
  const { data: remote } = await supabase
    .from('notes')
    .select('title,body,created_at,project_id')
    .order('created_at', { ascending: false })
    .limit(100);
  const remoteKeys = new Set(
    (remote || []).map((n) =>
      noteKey({ title: n.title, body: n.body, time: Date.parse(n.created_at) }),
    ),
  );
