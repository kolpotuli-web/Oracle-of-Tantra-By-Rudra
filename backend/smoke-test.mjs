...
const port = 4317;
const env = { ...process.env, PORT: String(port), FRONTEND_ORIGIN: `http://localhost:${port}`, OPENAI_API_KEY: '' };
const child = spawn(process.execPath, ['server.js'], { cwd: new URL('.', import.meta.url).pathname, env, stdio: ['ignore', 'pipe', 'pipe'] });
const stop = () => { if (!child.killed) child.kill('SIGTERM'); };
process.on('exit', stop);

function waitForStart() {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('Timed out waiting for backend startup.')), 10000);
    const onData = chunk => {
...
async function main() {
  ...
  if (invalid.status !== 400) throw new Error(`/api/chat validation returned ${invalid.status}, expected 400.`);

  const page = await fetch(`http://localhost:${port}/`);
  if (!page.ok) throw new Error(`Frontend returned ${page.status}.`);
  const html = await page.text();
  if (!html.includes('<title>The Oracle of Tantra</title>')) throw new Error('Frontend smoke check failed.');
