...
const allowedOrigins = configuredOrigin.split(',').map(x => x.trim()).filter(Boolean);

app.disable('x-powered-by');
app.set('trust proxy', 1);
app.use(helmet({ contentSecurityPolicy: false }));
app.use(cors({ origin: (origin, cb) => { if (!origin || allowedOrigins.includes(origin)) return cb(null, true); return cb(new Error('Origin not allowed')); }, methods: ['GET', 'POST'], allowedHeaders: ['Content-Type'] }));
app.use(express.json({ limit: '96kb' }));

const chatLimiter = rateLimit({ windowMs: 60 * 1000, limit: 20, standardHeaders: true, legacyHeaders: false, message: { error: 'Too many requests. Please wait a moment and try again.' } });
...
const openai = process.env.OPENAI_API_KEY ? new OpenAI({ apiKey: process.env.OPENAI_API_KEY, timeout: 45000, maxRetries: 2 }) : null;

async function askOpenAI(messages, language) {
  const response = await openai.chat.completions.create({
    model: process.env.OPENAI_MODEL || 'gpt-5.6-luna',
    temperature: 0.25,
    messages: [{ role:'system', content: `${systemPrompt}\n\nRespond entirely in ${languageNames[language] || 'English'}, while preserving original Sanskrit names where useful.` }, ...messages]
  });
...
async function askPollinations(messages, language) {
  const response = await fetch(process.env.POLLINATIONS_URL || 'https://text.pollinations.ai/', {
    ...
    body:JSON.stringify({ model:process.env.POLLINATIONS_MODEL || 'openai', messages:[{role:'system',content:systemPrompt},...messages,{role:'user',content:`Respond entirely in ${languageNames[language] || 'English'}.`}] }),
...
app.get('/api/health', (_req,res)=>res.json({ok:true, provider:openai?'openai':'pollinations', version:'1.1'}));
app.post('/api/chat', chatLimiter, async (req,res)=>{
  const parsed=chatSchema.safeParse(req.body);
  if(!parsed.success) return res.status(400).json({error:'Invalid request. Send at least one valid chat message.'});
  try{
